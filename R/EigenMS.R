...
#   ncompl - number of complete peptides with no missing observations
  ...
  if(n.u.treatment > 1) { 
    ...
    # Create the model matrix without attaching
    mod.c = model.matrix(lm.fm$lm.formula, data=TREAT, eval(parse(text=lm.fm$lm.params))) 
    Y.c = as.matrix(complete)
    
    # Construct the formula using the data frame directly
    formula1 = as.formula(paste('t(Y.c) ~', paste(names(TREAT), collapse = " + ")))
    fit_lmAll = lm(formula1, data = TREAT)
  ...
  retval = list(m=m, treatment=treatment, my.svd=my.svd,
            ...
            complete=complete, toplot1=toplot1, Tk=Tk, Rk=Rk, ncompl=numcompletepep,
            grp=grp) 
  return(retval)
}
...
eig_norm2 = function(rv) { 
  ...
  return(list(normalized=final, norm_m=y_rescaled, eigentrends=V0, rescrange=maxrange, 
              norm.svd=toplot3, exPeps=exPeps, Nk=Nk)) 
